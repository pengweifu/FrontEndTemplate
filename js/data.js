$(function(){
  if ($('.swiper-wrapper:not(.manual)').length>0) {
    ...
    $.ajax({
      ...
      for (var ii = 0; ii < $('.swiper-wrapper:not(.manual)').length; ii++) {
        var root=$('.swiper-wrapper:not(.manual)').eq(ii).parent().parent().attr('class');
        root='.'+root+' ';
  ...
  if ($('.m-list').length>0) {
    ...
    $.ajax({
      ...
      for (var i in data.list) {
        $('.m-list').append(tmp.replace(/\{\$field\.url\}/g,data.list[i].url).replace(/\{\$field\.title\}/g,data.list[i].title).replace(/\{\$field\.img\}/g,data.list[i].img).replace(/\{\$field\.date\}/g,data.list[i].date));
      }
    }).fail(function(status, jqXHR){console.log(status);});
  }
